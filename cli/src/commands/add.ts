...
export const add = new Command()
  ...
  .option('--routes', 'Inject module routes into router (if supported)', false)
  .option('--guards', 'Inject auth guards into router (for auth bundles)', false)
  .addHelpText('after', `
Examples:
  ...
  .action(async (components: string[], options) => {
    try {
      ...
      for (const componentName of components) {
        ...
        // Optional: inject routes/guards for known bundles
        if (options.routes || options.guards) {
          await tryInjectForBundle(componentName, cwd, {
            routes: options.routes ?? false,
...
async function tryInjectForBundle(
  name: string,
  cwd: string,
  opts: { routes: boolean; guards: boolean },
): Promise<void> {
  const lower = name.toLowerCase()
  const importLines: string[] = []
  const spreadLines: string[] = []
  let guardBlock: string | undefined
  ...
  if (opts.routes) {
    ...
    if (lower.includes('dashboard')) {
      importLines.push('import { dashboardRoutes } from \'@/modules/dashboard/routes\'')
      spreadLines.push('...dashboardRoutes,')
    }
    if (lower.includes('user')) {
      importLines.push('import { userRoutes } from \'@/modules/user/routes\'')
      spreadLines.push('...userRoutes,')
    }
    if (lower.includes('settings')) {
      importLines.push('import { settingsRoutes } from \'@/modules/settings/routes\'')
  ...
  if (opts.guards && lower.includes('auth')) {
    guardBlock = [
  ...
  if (importLines.length > 0 || spreadLines.length > 0 || guardBlock) {
