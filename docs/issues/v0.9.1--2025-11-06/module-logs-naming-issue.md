...
**Status:** Naprawiono w `vue-blocks-registry` w commit `117cfa9`

Moduł został przemianowany z `logs` na `logs-management` zgodnie z rekomendacją.

**Zmiany:**
- Katalog: `registry/modules/logs-management/`
- Wpisy w registry.json: `logs-components`, `logs-pages`, `logs-routes`, `logs-i18n`, `logsFull`
- Wszystkie ścieżki w registry zaktualizowane

Problem nie występuje już w `vue-blocks-registry`. Wymaga implementacji w `fastapi-blocks-registry`.
